    ...
    methods: {
        ...
        createElementFromVNode(vnode, createElement, fieldValue) {
            ...
            if (vnode.data.staticClass)
                elementConf.class = vnode.data.staticClass;
            if (vnode.componentOptions)
                elementConf.on = vnode.componentOptions.listeners || {};
            if (vnode.data.attrs.value == fieldValue)
                elementConf.domProps.selected = true;
            return createElement(vnode.tag, elementConf, children);
        },
    },
    render(createElement) {
        const { values, change } = this.vuemik;
        ...
        if (this.$slots.default) {
            children = this.$slots.default.map((vnode) => {
                return this.createElementFromVNode(
                    vnode,
                    createElement,
                    fieldValue
                );
