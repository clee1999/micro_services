...
<template>
  <div>
    <v-container>
                ...
                <v-card-text>
                  <div>
                    <p>
                      ...
                      <v-icon color="black" small>mdi-clock-time-eight</v-icon>
      ...
      <v-row v-for="j in justify" :key="j" :justify="j">
        <div :key="index" v-for="(reservation, index) in lastReservations">
          <v-card class="mx-auto my-12 mr-5" max-width="374">
            <div class="card-group">
              <div class="text-center">
                <v-card-text>
              ...
              <div class="card-main-content rounded-lg text-center pa-4">
                ...
                <p>{{ reservation.doctor.metier }}</p>
                <p class="mt-3">{{ reservation.doctor.address }}</p>
              </div>
            </div>
          </v-card>
        </div>
...
export default {
  ...
  methods: {
    deleteReservation(reservation) {
      ...
      axios
        .delete("/reservations/" + reservation["@id"].split("/")[3])
        .then(() => {
      ...
      axios
        ...
        .then((response) => {
          response.data["hydra:member"].forEach((reservation) => {
            if (reservation.patient == "/api/users/" + this.user.id) {
